...
class BybitTradingExecutor:
    ...
    def __init__(self, 
        ...
        Args:
            ...
            max_risk_per_trade: Maximum risk per individual trade (1% strict limit)
            max_portfolio_risk: Maximum total portfolio risk
            min_confidence: Minimum signal confidence to trade
            dynamic_take_profit: Use dynamic take profit based on signal quality
        """
    ...
    def calculate_dynamic_take_profit(self, signal: TradingSignal, current_price: float) -> float:
        """
        Calculate dynamic take profit based on signal quality
        Uses same quality scoring as ICT monitor for consistency
        ...
        Returns:
            Take profit price
        """
        # Calculate quality score (same as ICT monitor)
        quality_score = (signal.confluence_score + signal.bias_strength) / 2
        
        # Dynamic RR ratios based on quality
        dynamic_rr_ratios = {
            'conservative': 2.0,  # 1:2 RR
            'balanced': 3.0,      # 1:3 RR  
            'aggressive': 4.0,    # 1:4 RR
            'maximum': 5.0        # 1:5 RR
        }
        
        # Select RR ratio based on quality
        if quality_score >= 0.85:
            rr_ratio = dynamic_rr_ratios['maximum']
            rr_type = "maximum"
        elif quality_score >= 0.75:
            rr_ratio = dynamic_rr_ratios['aggressive'] 
            rr_type = "aggressive"
        elif quality_score >= 0.65:
            rr_ratio = dynamic_rr_ratios['balanced']
            rr_type = "balanced"
        else:
            rr_ratio = dynamic_rr_ratios['conservative']
            rr_type = "conservative"
        ...
        if signal.stop_loss:
            ...
            # Fallback: use default 1% stop with dynamic RR
            default_stop_pct = 0.01  # 1%
            profit_pct = default_stop_pct * rr_ratio
        ...
        logger.debug(f"🎯 Dynamic TP for {signal.symbol}: {take_profit:.6f} ({rr_type} 1:{rr_ratio} RR, quality: {quality_score:.3f})")
        return take_profit
    ...
    async def execute_signal(self, signal: TradingSignal) -> Optional[TradeExecution]:
        ...
        try:
            ...
            trade_execution = TradeExecution(
                ...
                stop_loss=signal.stop_loss,
                take_profit=take_profit_price  # Use dynamic or original TP
            )
