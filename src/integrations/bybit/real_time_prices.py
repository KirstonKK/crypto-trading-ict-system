    ...
    async def _process_ticker_update(self, data: Dict):
        ...
        try:
            ...
            if symbol and ticker_data:
                # Parse the WebSocket ticker data
                ...
                if price_data['price'] > 0:
                    ...
                    self.prices[symbol] = price_data
                    logger.debug(f"✅ Updated {symbol}: ${price_data['price']:,.4f}")
                else:
                    logger.warning(f"⚠️ Received invalid price for {symbol}: {price_data['price']}")
            else:
    ...
    def _parse_ws_ticker_data(self, ticker: Dict, symbol: str) -> Dict:
        ...
        try:
            ...
            if isinstance(last_price_str, str):
                price_value = float(last_price_str) if last_price_str and last_price_str != '0' else 0.0
            else:
